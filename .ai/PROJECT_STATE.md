...
- **Current state:** P0 + P1 complete; **P2 complete (6/6)** and committed. **Phase 3-A is complete (Issues 1-8)**. **Showcase assets + reproducibility baseline are complete**. **Context Compaction C0-C5 are complete**. **Failure-aware Negative Memory Injection I1-I6 are complete** and **I7 compaction negative retained is now the selected next implementation target**. **Phase 3.5 SDK/LangGraph adapter/CLI is complete through S6** per `docs/design/SDK_ADAPTER_PLAN.md`. **ROADMAP §7 "完整 6 策略对比 + benchmark 落库" is complete through Task 11 full regression/reproducibility/project-memory sync.** **ROADMAP §13 Security & Consistency Hardening is complete through H18** and post-review hardened. **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology is complete through P10:** provider registry/factory/DI, runtime/retrieval/replay embedding provider integration, retrieval-policy-v2 provider snapshots, deterministic benchmark provider isolation, contract-only judge provider, controlled memory-key ontology, writer/resolver/runtime ontology migration, and LLM extraction ontology normalization are implemented and verified.
- **Last updated:** 2026-06-14 (I7.1-I7.4 implemented and review-fixed. `RetainedNegativeEvidence` now exists as a dedicated safe metadata DTO and `to_retained_negative_evidence(...)` converts only from `NegativeEvidence.safe_text` with redaction defense-in-depth. `ContextCompactionLog` / `PendingCompactionLog` carry `retained_negative_evidence`; SQL ORM adds a non-null JSONB column with `sa.text("'[]'::jsonb")`; SQL writes use `model_dump(mode="json")` and reads use `RetainedNegativeEvidence.model_validate(...)` with old/missing rows mapping to `[]`; Alembic migration `0007_i7_retained_negative_evidence` uses actual head `0006_security_consistency_hardening`. I7.3 now retains dropped standard `avoided_attempts` / `source="negative_evidence"` blocks as metadata through negative-evidence maps without forcing prompt injection. I7.4 surfaces retained negative evidence in replay metrics, observability summary/per-access metrics, JSON/Markdown/HTML reports, and redacted trace bundles. Review fixes keep `negative_evidence_block_count` as actual prompt blocks by excluding retained metadata and defensively redact retained negative evidence in report JSON/Markdown/HTML including replay payloads. Latest I7.3/I7.4 verification: packer negative suite **20 passed**; targeted replay/report/bundle suite **11 passed**; affected retrieval+observability+storage suite **194 passed, 1 skipped**; compileall over `apps/api/app` passed. I7.5 benchmark `case_13_compaction_retains_negative_lesson` remains next.)
...
Post-P3A **Context Compaction (ROADMAP §9)** is complete through C5, **Failure-aware Negative Memory Injection** is complete through I6, **Phase 3.5 SDK/LangGraph adapter/CLI** is complete through S6, **ROADMAP §7 6-strategy benchmark expansion + eval-table persistence** is complete through Task 11, **ROADMAP §13 Security & Consistency Hardening** is complete through H18, and **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology** is complete through P10.

**Latest completed full roadmap target:** ROADMAP §10/§11 **Provider Registry + Controlled Memory Key Ontology** is complete through P10, including benchmark deterministic provider isolation, provider snapshot conformance, full regression/reproducibility closeout, and final review fixes for fixed 256-dim provider embeddings, package-manager correction semantics, alias survivor canonicalization, ontology schema coverage, summarizer provider wiring, and free-form default type/scope. **Current implementation target:** **I7 compaction-negative retained facts** (formerly deferred) is selected and complete through I7.1-I7.4, using `docs/design/I7_COMPACTION_NEGATIVE_RETENTION_PLAN.md` as the authoritative implementation plan. Continue with I7 only; next is I7.5 benchmark case 13; do not pull in Phase 4 async/lifecycle/governance or TS/MCP/IDE integration work during this slice.

## Selected Next Implementation Target (I7 Compaction Negative Retention — selected 2026-06-13)
...
- **Planned task order:** I7.1 retained-negative DTO/conversion ✅; I7.2 dedicated compaction-log field/migration/SQL mapping ✅; I7.3 packer retention for dropped negative evidence ✅; I7.4 replay/metrics/reports/trace-bundle surfacing ✅; I7.5 benchmark `case_13_compaction_retains_negative_lesson` next; I7.6 docs/project-memory closeout.
- **Required verification:** targeted RED/GREEN per task; migration tests; retrieval/observability/benchmark affected suite; compileall; full `uv run --extra dev pytest -q`; deterministic benchmark; `bash scripts/reproduce.sh`; unsafe-marker scan over generated reports/bundles for `rm -rf`, `/prod`, `sk-`, `password`, and `Authorization`.
- **Future plans deliberately out of scope:** `docs/design/PHASE4_PLATFORM_PLAN.md` (async/lifecycle/governance) and `docs/design/INTEGRATIONS_PLAN.md` (TS SDK/MCP/IDE) remain future work after I7.
...
ROADMAP §13 Security & Consistency Hardening 已完成 H1-H18，ROADMAP §10 Provider Registry + §11 Controlled Memory Key Ontology 已完成 P1-P10，**I7 compaction-negative retained facts 已完成 I7.1-I7.6 并通过 closeout verification**。下一步建议从 post-I7 候选中择一：real Reflection/Forgetting scheduler（替换 deterministic `variant_3` reflection-lite，最贴近当前 benchmark 缺口）、Phase 4 async/lifecycle/governance、多租户 readiness，或 TypeScript/MCP/IDE integrations。
