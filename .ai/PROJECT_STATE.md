...
- **Current state:** P0 + P1 complete; **P2 complete (6/6)** and committed. **Phase 3-A is complete (Issues 1-8)**. **Showcase assets + reproducibility baseline are complete**. **Context Compaction C0-C5 plus C6/I7 retained-negative metadata are complete**. **Failure-aware Negative Memory Injection I1-I7 are complete**. **Phase 3.5 SDK/LangGraph adapter/CLI is complete through S6** per `docs/design/SDK_ADAPTER_PLAN.md`. **ROADMAP §7 "完整 6 策略对比 + benchmark 落库" is complete through Task 11 full regression/reproducibility/project-memory sync.** **ROADMAP §13 Security & Consistency Hardening is complete through H18** and post-review hardened. **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology is complete through P10.** **Phase 4 async/lifecycle/governance is complete through P4-D4 and final P4-D review hardening.** **Integrations INT-A TypeScript SDK, INT-B MCP Server, and INT-C MCP config templates / IDE thin-layer decision are complete and cross-reviewed:** Bun workspace packages `@memtrace/sdk` and `@memtrace/mcp-server`, strict TypeScript DTOs/client/errors, current Phase 4 HTTP surface coverage, mock contract smoke, optional real-service smoke, public-package TS example, stdio MCP tools over the SDK, redacted/capped MCP replay/report outputs, Claude Code/Cursor MCP config templates, recursive/broadened JSON secret-key MCP redaction hardening, unknown-tool error redaction/capping, HTTP(S)-only no-userinfo base URL validation, cwd-stable package tests, and stale superpowers plan closeout are implemented and verified. **Current selected next target:** none inside INT-C; dedicated IDE package remains deferred until MCP adoption feedback. Next roadmap candidates are OpenTelemetry/OpenInference exporter, advanced UI/dashboard, admin/manual governance depth, or advanced retrieval/storage work.
- **Last updated:** 2026-06-14 (Completed full cross-INT review before commit. Review found no remaining P0 defects after hardening; fixed one MCP output security gap by broadening `redactToolText(...)` to parse whole JSON and redact common secret-bearing keys such as `authorization`, bare `token`, `client_secret`, `secret_key`, `id_token`, and `*_token` / `*_secret` / `*_credential`; fixed the TS example to import public `@memtrace/sdk` and added `tsconfig.base.json` path mapping so root/package typecheck stays aligned; updated `AGENTS.md` production-code boundaries to include `packages/ts-sdk` and `packages/mcp-server`; marked INT-A/INT-B superpowers plans as historical/completed with next action now selecting the next roadmap target. Verification with temporary Bun: `npm exec --yes --package bun -- bun test packages/mcp-server/test/tools.test.ts` -> **16 passed**; `npm exec --yes --package bun -- bun run typecheck` passed; `npm exec --yes --package bun -- bun test` -> **27 passed, 1 skipped**; package-local `cd packages/mcp-server && npm exec --yes --package bun -- bun test test` -> **18 passed**; `npm exec --yes --package bun -- bun test packages/ts-sdk/test` -> **9 passed, 1 skipped**; forbidden `node_modules` / `*.tsbuildinfo` / npm/pnpm/yarn lockfiles are not tracked. Default local/dev/benchmark behavior remains no-auth/no-quota unless explicitly enabled. Integrations stay thin over the stable `/v1` HTTP contract and do not duplicate Python runtime/retrieval/gate/context-packing semantics.)
...
Post-P3A **Context Compaction (ROADMAP §9)** is complete through C5 plus C6/I7 retained-negative metadata, **Failure-aware Negative Memory Injection** is complete through I7, **Phase 3.5 SDK/LangGraph adapter/CLI** is complete through S6, **ROADMAP §7 6-strategy benchmark expansion + eval-table persistence** is complete through Task 11, **ROADMAP §13 Security & Consistency Hardening** is complete through H18, and **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology** is complete through P10.

**Latest completed roadmap slice:** INT-C MCP config templates / IDE thin-layer decision is complete after INT-A TypeScript SDK and INT-B MCP Server. INT-A implemented `@memtrace/sdk` as a thin fetch client over `/v1` with current Phase 4 contracts: 400/422/401/403/404/429 mapping, workspace-scoped session flush, async extraction fields, path-authoritative complete-run, memory versions/conflicts, and mock/optional real-service contract smoke. INT-B implemented `@memtrace/mcp-server` as a stdio MCP adapter over that SDK with redacted/capped outputs. INT-C added Claude Code/Cursor MCP config templates using env placeholders and explicitly deferred a dedicated IDE package. Keep future integrations as thin clients over HTTP/SDK semantics and do not import Python runtime code or bypass `MemoryRuntime`.
...
- **Next recommended action:** choose the next roadmap target. Likely candidates are OpenTelemetry/OpenInference exporter, advanced UI/dashboard, admin/manual governance depth, or advanced retrieval/storage work. Dedicated IDE extension remains deferred until MCP adoption feedback exists.
...
- **Next recommended action:** INT-C is complete. Choose the next roadmap target; likely candidates are OpenTelemetry/OpenInference exporter, advanced UI/dashboard, admin/manual governance depth, or advanced retrieval/storage work. Dedicated IDE extension remains deferred until MCP adoption feedback exists.
...
- **Historical next action after §10/§11:** choose the next roadmap target. This has since been superseded: I7 compaction-negative retained facts was selected, I7.1-I7.4 are complete, and I7.5 benchmark case 13 is the current next action.
...
- **ROADMAP 丰富:** §1.1 现在区分已修复、§13 已关闭、以及仍明确后置的非 §13 待办；§13 安全与一致性加固（13.1 安全闭环 / 13.2 一致性并发 / 13.3 精度健壮性 / 13.4 横切保障）已完成；§11 记录「本体作为单一真相源」以消除三处单值语义漂移；推荐推进顺序已把完成态的 §13 置于 Provider Registry 之前并将下一步重新指向 §10/§11。
...
ROADMAP §13 Security & Consistency Hardening 已完成 H1-H18，ROADMAP §10 Provider Registry + §11 Controlled Memory Key Ontology 已完成 P1-P10，I7 compaction-negative retained facts 已完成 I7.1-I7.6；Phase 4 async/lifecycle/governance 已完成 P4-A1-P4-A5、P4-B1-P4-B4、P4-C1-P4-C2、P4-D1-P4-D4，并通过 full regression/reproducibility；**Integrations INT-A TypeScript SDK、INT-B MCP Server、INT-C MCP config templates / IDE thin-layer decision 均已完成并验证。下一步需从剩余 ROADMAP 中选择：OpenTelemetry/OpenInference exporter、advanced UI/dashboard、admin/manual governance depth、或 advanced retrieval/storage。专用 IDE 扩展继续后置，直到 MCP adoption 产生真实反馈。继续保持 integrations 为 thin clients over `/v1`，不得导入 Python runtime/database 或复制 retrieval/gate/context-packing 语义。**
