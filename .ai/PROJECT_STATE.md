...
- **Last updated:** 2026-06-13 (Provider Registry + Key Ontology P8-P10 completed with TDD, closeout, and final code-review hardening. Benchmark `_run_case(...)` injects `deterministic_provider_registry()` even when real-provider env vars are set; provider conformance asserts access policy snapshots are non-secret and retrieval-relevant; final review fixed settings-derived embedding dimensions to the fixed 256-dim pgvector contract and package-manager corrections such as `npm -> bun`; tests now lock ontology schema completeness and summarizer provider factory wiring. Final verification: affected provider/ontology/runtime/retrieval/replay/benchmark/conformance suite **322 passed**; full regression **460 passed, 1 skipped**; reproduce: `acceptance.passed=true (12/12 checks true)`. Next roadmap target is not selected; choose from deferred I7, real Reflection/Forgetting scheduler, TS/MCP/IDE integrations, or Phase 4 governance.)
