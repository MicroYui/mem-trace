...
- **Current state:** P0 + P1 complete; **P2 complete (6/6)** and committed. **Phase 3-A is complete (Issues 1-8)**. **Showcase assets + reproducibility baseline are complete**. **Context Compaction C0-C5 plus C6/I7 retained-negative metadata are complete**. **Failure-aware Negative Memory Injection I1-I7 are complete**. **Phase 3.5 SDK/LangGraph adapter/CLI is complete through S6** per `docs/design/SDK_ADAPTER_PLAN.md`. **ROADMAP §7 "完整 6 策略对比 + benchmark 落库" is complete through Task 11 full regression/reproducibility/project-memory sync.** **ROADMAP §13 Security & Consistency Hardening is complete through H18** and post-review hardened. **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology is complete through P10.** **Phase 4 async/lifecycle/governance is complete through P4-D4 and final P4-D review hardening.** **Integrations INT-A TypeScript SDK, INT-B MCP Server, and INT-C MCP config templates / IDE thin-layer decision are complete and cross-reviewed. R1 Release Readiness & Public Adoption is complete:** R1-A0/A1/A2/A3/B1/B2/B3/C1/C2 are done, including release checklist, CI, package metadata, smoke docs, release hygiene, benchmark/reproduce closeout, and ROADMAP/`.ai` sync. Dedicated IDE package remains deferred until MCP adoption feedback. **Next recommended feature slice:** OpenTelemetry/OpenInference exporter, unless adoption feedback reprioritizes maintenance scheduler completion, admin workflow, advanced UI/dashboard, or advanced retrieval/storage.
- **Last updated:** 2026-06-14 (Fixed CI follow-up failures after R1 closeout. Root cause: dogfood subprocess smoke tests inherited the uv test interpreter but not pytest's `pythonpath`, so fresh CI virtualenv subprocesses could not import `memtrace_sdk`; release hygiene also flagged the documented placeholder `MEMTRACE_API_KEY=replace-with-a-dev-token`. Fix: dogfood subprocess tests now pass explicit `PYTHONPATH=apps/api:packages/python-sdk/src`, and release hygiene allows `replace-with-*` placeholder values while still scanning public docs/examples for real secret markers. Latest targeted verification: fresh uv env dogfood tests **3 passed** using `UV_PROJECT_ENVIRONMENT=/tmp/memtrace-ci-repro-venv`; `bash scripts/check-release-hygiene.sh` -> `release hygiene checks passed`; script syntax checks passed. Previous full R1 verification remains valid and should be rerun before final completion if additional changes are made.)
...
Post-P3A **Context Compaction (ROADMAP §9)** is complete through C5 plus C6/I7 retained-negative metadata, **Failure-aware Negative Memory Injection** is complete through I7, **Phase 3.5 SDK/LangGraph adapter/CLI** is complete through S6, **ROADMAP §7 6-strategy benchmark expansion + eval-table persistence** is complete through Task 11, **ROADMAP §13 Security & Consistency Hardening** is complete through H18, **ROADMAP §10/§11 Provider Registry + Controlled Memory Key Ontology** is complete through P10, **Phase 4 async/lifecycle/governance** is complete through P4-D4, **INT-A/INT-B/INT-C integrations** are complete, and **R1 Release Readiness & Public Adoption** is complete. R1 makes the project externally understandable, runnable, connectable, reproducible, and release-ready without adding new runtime semantics.

**Latest completed roadmap slice:** R1 Release Readiness & Public Adoption is complete after R1-C1/R1-C2 closeout. The default CI covers Python compile/full pytest, Bun typecheck/tests, and release hygiene without requiring optional services by default; README/user docs cover no-network onboarding, local HTTP/TS/MCP paths, benchmark/reproduce, deployment notes, and maintainer release checklist. Keep future integrations as thin clients over HTTP/SDK semantics and do not import Python runtime code or bypass `MemoryRuntime`.

**Selected next implementation target:** choose the next roadmap slice. The recommended next feature is **OpenTelemetry/OpenInference exporter** because R1 established public onboarding and reproducibility, and exporter value is highest once users can run and inspect the project. Keep JS packages `private: true` unless publishing is explicitly approved, avoid introducing a `dist` build pipeline by default, keep optional Redis/PostgreSQL/LLM/live HTTP checks gated, and do not advertise OTel/LangSmith/Phoenix/Langfuse support as available until an exporter exists.
...
- **Next recommended action:** select and plan OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area.
...
- **Next recommended action:** select and plan OpenTelemetry/OpenInference exporter, unless adoption feedback reprioritizes maintenance scheduler completion, admin/manual governance depth, advanced UI/dashboard, or advanced retrieval/storage.
...
- **Historical next action after R1-B3:** R1-C1 release checklist, followed by R1-C2 full verification and project-memory/ROADMAP closeout; superseded by completed R1-C1/R1-C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area.
...
- **Historical next action after R1-A0/A1/A2:** R1-A3 demo flow consolidation / smoke coverage; superseded by completed R1-A3/B1/B2/B3/C1/C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area.
...
- **Historical next action after cross-INT review:** choose the next roadmap target; superseded by R1 selection and completed R1-A0/A1/A2/A3/B1/B2/B3/C1/C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area. Dedicated IDE extension remains deferred until MCP adoption feedback exists.
...
- **Historical next action after INT-C review:** choose the next roadmap target; superseded by R1 selection and completed R1-A0/A1/A2/A3/B1/B2/B3/C1/C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area. Dedicated IDE extension remains deferred until MCP adoption feedback exists.
...
- **Historical next action after INT-B:** INT-C, then choose the next roadmap target; superseded by completed INT-C, R1 selection, and completed R1-A0/A1/A2/A3/B1/B2/B3/C1/C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area. Dedicated IDE extension remains deferred until MCP adoption feedback exists.
...
- **Historical next action after §10/§11:** choose the next roadmap target. This was superseded by completed I7 compaction-negative retained facts, then completed Phase 4 and integrations, then completed R1-A0/A1/A2/A3/B1/B2/B3/C1/C2. Current next recommended action is selecting OpenTelemetry/OpenInference exporter unless adoption feedback reprioritizes another roadmap area.
...
- **ROADMAP 丰富（historical）:** §1.1 区分已修复、§13 已关闭、以及仍明确后置的非 §13 待办；§13 安全与一致性加固（13.1 安全闭环 / 13.2 一致性并发 / 13.3 精度健壮性 / 13.4 横切保障）完成后曾把下一步重新指向 §10/§11。该历史顺序已被后续进展 supersede：§10/§11、Phase 4、Integrations、R1 均已完成；当前下一推荐为 OpenTelemetry/OpenInference exporter。
...
R1 Release Readiness & Public Adoption is complete. **R1-A0/A1/A2/A3/B1/B2/B3/C1/C2 are complete and verified**, including release checklist, benchmark/reproduce closeout, release hygiene, and project-memory sync. Recommended next feature slice: **OpenTelemetry/OpenInference exporter** unless adoption feedback reprioritizes maintenance scheduler completion, admin workflow, advanced UI/dashboard, or advanced retrieval/storage. Dedicated IDE extension remains deferred until MCP adoption feedback shows editor-specific needs. Continue keeping integrations as thin clients over `/v1`, without importing Python runtime/database or duplicating retrieval/gate/context-packing semantics.
