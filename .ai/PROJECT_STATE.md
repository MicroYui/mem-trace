...
## Latest Session (2026-06-15 — full review fixes + Phase 3-B Dashboard UI + RRF retrieval fusion)
...
- **Phase 3-B Dashboard UI (committed `662851b`, pushed):** Added `apps/api/app/api/dashboard_ui.py` (a self-contained HTML page: inline CSS, vanilla JS, no build step / external JS / CDN) served at `GET /v1/dashboard/ui` (`include_in_schema=False`, unauthenticated shell; data APIs still enforce auth). The page fetches `/v1/dashboard/tables` and renders the observability summary cards, per-strategy + benchmark_summary tables, and runs/accesses/profile tables. All dynamic values rendered via `textContent`/DOM (no `innerHTML` of untrusted data). Token field sends `Authorization: Bearer` + `X-API-Key`. README documents it and the roadmap note no longer lists a dashboard as pure future work.
- **Advanced retrieval — RRF fusion (committed `aae0937`, pushed):** Implemented the dependency-free deterministic part of ROADMAP §4 multi-signal fusion. New settings `MEMTRACE_RETRIEVAL_FUSION` (`linear` default / `rrf`) + `MEMTRACE_RETRIEVAL_RRF_K` (default 60). `RetrievalController._select_candidates` does a two-pass Reciprocal Rank Fusion over lexical + vector rankings when enabled (`1/(k+rank)`, deterministic descending/`memory_id` tie-break, project-constraint floor preserved). `build_policy_snapshot(..., fusion=, rrf_k=)` records fusion mode **only** for non-default RRF, so existing linear policy hashes / replay snapshots stay byte-stable. Heavy ES/OpenSearch/Neo4j hybrid retrieval, graph provenance, multi-hop, and task_intent ranking-profiles remain deferred per the §4 trigger conditions.
- **Verification (all three slices):** compileall passed; full `uv run --extra dev pytest -q` → **750 passed, 2 skipped**; `bash scripts/reproduce.sh` → `acceptance.passed=true (13/13 checks true)`; `bash scripts/check-release-hygiene.sh` → passed; `git diff --check` clean; `npm exec --yes --package bun -- bun run typecheck` passed; `bun test` → 31 pass, 1 skip; dashboard JS passed a browser-target syntax build check.
