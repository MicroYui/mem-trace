...
### ADR-033: Select Phase 3-B Showcase Dashboard as the next implementation target

- **Date:** 2026-06-17
- **Status:** accepted
- **Context:** MemTrace now has a broad verified backend/runtime platform, Python/TypeScript/MCP integration surfaces, release-ready docs, telemetry export, maintenance/admin governance depth, a small built-in static dashboard viewer, and a default-off RRF retrieval micro-slice. The remaining gap for public adoption is visual inspectability: the core value is difficult to grasp from JSON, Markdown reports, and the minimal `/v1/dashboard/ui` shell alone.
- **Decision:** Select `docs/design/PHASE3B_DASHBOARD_PLAN.md` as the next implementation plan. Build a polished React + TypeScript dashboard in `apps/web` over `@memtrace/sdk` and existing read-only `/v1` APIs. The dashboard should be a real product surface, not a landing page: overview/run gallery, run timeline, state tree, access replay, gate/memory-flow visualization, benchmark lab, memory atlas, read-only owner-gated ops panels, and fixture-backed showcase mode for screenshots/offline demos. The first slice is WEB-A scaffold/build boundary.
- **Consequences:** Future resume sessions should start with WEB-A rather than selecting a new roadmap target. `/v1/dashboard/ui` remains a lightweight static viewer and should not be expanded into the full frontend. Any backend additions for the dashboard must be bounded read-only projections that preserve existing authz, quota, redaction, workspace isolation, and deterministic benchmark behavior. Phase 5 ES/Neo4j/query-planner/multi-hop work, admin mutation UI, distributed scheduler lease, and dedicated IDE packages remain separate future targets.
- **Sources:** `docs/design/PHASE3B_DASHBOARD_PLAN.md`, `docs/design/ROADMAP.md` §3-B, `.ai/PROJECT_STATE.md`, `.ai/IMPLEMENTATION_PLAN.md`.
