...
Phase 3-A scope: Retrieval Replay, eval tables, Quality/Safety profiler metrics, expanded profiler phases, dashboard-table extension, and static JSON/Markdown/HTML observability reports. Use `P3A_IMPLEMENTATION_PLAN.md` as the concrete implementation plan before touching code. **Issue 1 is complete** (access fidelity + eval persistence schema), **Issue 2 is complete** (side-effect-free retrieval trace pipeline), **Issue 3 is complete** (replay service + deterministic diff semantics), **Issue 4 is complete** (replay/observability APIs), **Issue 5 is complete** (Quality/Safety metrics + profiler phase expansion), and **Issue 6 is complete** (dashboard table extension); the next concrete slice is **Issue 7: JSON/Markdown/HTML observability reports**.
