...
Context Compaction (ROADMAP §9) is complete through C5 per `docs/design/CONTEXT_COMPACTION_PLAN.md`: `PackResult`, budget-aware `compacted_constraints` + `compaction_notice`, durable `ContextCompactionLog` with observability/replay wiring, rule/LLM `SummarizerProvider`, config-gated rolling history summary, and retention-quality benchmark/report/replay sync. Next recommended slices are Phase 3.5 SDK/LangGraph adapter (§6) or 6-strategy benchmark expansion (§7) in `docs/design/ROADMAP.md`.
