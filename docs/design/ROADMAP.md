...
- [ ] **人工审核 memory conflict 管理后台**（admin）。下一计划会先做 owner-gated HTTP admin workflow（mark false positive / choose winner and supersede losers），但不做 React 管理后台 UI；完整可视化 admin console 继续后置。
...
- [ ] **多路候选融合 RRF/加权**（vector + BM25 + graph）+ 按 task_intent 切换 ranking_profiles。出处：architecture §6.5。**部分完成**：已实现 deterministic、无外部依赖的 Reciprocal Rank Fusion (RRF) 融合 lexical + vector 两路排名，默认仍为 `linear` 加权（benchmark/replay 行为不变），通过 `MEMTRACE_RETRIEVAL_FUSION=rrf` 切换，policy snapshot 记录 fusion 模式；BM25/graph 路与 task_intent ranking_profiles 仍后置（依赖 ES/Neo4j 触发条件）。
