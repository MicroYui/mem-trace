...
10. **I7 compaction-negative retained facts 已完成；后续候选为 Phase 4 异步基建 + 生命周期 + 多租户治理 / TS-MCP-IDE integrations**：默认大项可回到 §3 全段（§3.1 Redis buffer/Celery → §3.2 Reflection/Forgetting 调度器 + 10 定时任务 + 审计日志 → §3.3 memory_versions/conflicts → §3.4 API Key/JWT/quota/redaction 完整多租户治理），也可显式选择 TypeScript/MCP/IDE adoption slice；real Reflection/Forgetting scheduler 仍需替换当前 deterministic `variant_3` reflection-lite。
