...
- [x] **[Medium] 负向证据（avoided_attempts）非受保护块**：预算紧张时「请勿重复某危险操作」的安全提示可能被丢弃。**已完成（2026-06-30，默认关闭）**：新增 config `protect_safety_negative_evidence`（默认 `False`）+ `pack_context(protect_safety_notices=...)`；packer `_is_safety_notice(block)` 识别 `sanitized_risk_notice` 模式块（`type=="avoided_attempts" AND source=="negative_evidence" AND reason endswith "_sanitized"`）。启用后这些安全提示并入受保护集合（drop 循环与 reserve-shrink 循环都用本地 `_protected` 谓词），预算紧张时不被丢弃；`raw_failed_attempt` / `outdated_warning` 仍为 ordinary。Controller 读取 setting 传入两处 pack_context，replay 复用同参。默认关闭故 `case_13` retained-negative 语义与 benchmark/reproduce 13/13 不变；启用时安全提示直接留在 prompt（其 metadata 仍由 I7 retained 作冗余）。出处：`retrieval/packer.py`。
- [x] **[Low] 其余（2026-06-30 closeout 复核）**：**已修复 / 已确认修复**——`tool_sensitive_present` 子串误判改为 token 边界匹配（`writer.py` `_TOOL_SENSITIVE_PATTERNS=[(?<![a-z])secrets?(?![a-z])]`，保留 `client_secret`/`api_secret`/`secret_key` 等复合凭据标识符并丢弃 `secretary` 类前缀误判；destructive/production 仍由既有 flag 覆盖）；`last_accessed_at` 已由 P4-B `bump_memory_access` 写入并被 `retention._recency` 消费（recency 不再是死字段）；报告路径 TOCTOU 已由 `reports._safe_output_dir` 逐段 symlink + resolved-root 校验覆盖；`stale_injected` 全程用 `datetime.now(timezone.utc)`，无 naive/aware 混比；summarizer episodic 内容基于已脱敏 event 构建并经上游 governance 脱敏。**`access_count` read-modify-write 竞态已闭环（2026-06-30 loop Step 1）**：`update_memory` 现默认 `preserve_access_bookkeeping=True`，对**语义与非语义**两条路径都从 stored/row-locked `before` 携带 `access_count`/`last_accessed_at`，使 `bump_memory_access`（SQL 服务端 `+1` / in-memory 单事件循环无 await）成为访问簿记的唯一权威，stale caller copy 不再回滚原子 bump；唯一显式重置点是 benchmark fairness restore（`runner._restore_workspace_memories` 传 `preserve_access_bookkeeping=False` 回退到种子快照）。in-memory 与 SQL 两处对称改动以保持 backend isomorphism。**CLI 与 evaluator 判定重复已消除（同上）**：`decide_action`/`contaminated`/`positive_blocks`/`negative_blocks` 下沉到 `app/runtime/context_actions.py`（SDK 已经经 `memtrace_sdk.types` 依赖的 `app.runtime` DTO 层），evaluator / demo / CLI 三处旧的逐字节拷贝改为复用单一真相源；CLI 经 `memtrace_sdk.context_actions` 再导出（与 `memtrace_sdk.types` 同构），不触碰 `app.benchmark` 包边界。**仍待办（价值最低，by-design）**——`raw_event_ids` 为反规范化缓存，真相源是 `agent_events` join（非竞态修复目标）。详见审查记录。
...
- [x] **Elasticsearch / OpenSearch 混合检索**（dense vector + BM25 + filter + valid_time + branch_status）。出处：architecture §8.2，「第一阶段推荐」但被 pgvector 替代。**default-off、degrade-safe 的 BM25 混合检索 slice 已完成**：新增 `app/retrieval/hybrid.py` 提供 `HybridBackend` 协议、确定性 `InMemoryBM25Backend`（真实 Okapi BM25，IDF 偏好稀有词，无网络、可复现）、可选 `ElasticsearchBM25Backend`（lazy-import `elasticsearch`，per-workspace index upsert+match query，缺依赖/端点或任何查询异常 → `available=False`/`{}` 干净降级），`build_hybrid_backend(settings)`。`MEMTRACE_RETRIEVAL_HYBRID_BACKEND=off|inmemory|elasticsearch|opensearch`（默认 `off`），启用时 BM25 作为第三信号线性融入 `rel = w_lex*lex + w_vec*vec + w_bm25*bm25`（lex/vec 权重按 `1-hybrid_weight` 缩放，blend 仍和为 1），候选带 `bm25_score`。`pyproject` 新增 `search`/`graph` 可选 extras。默认 `off` → 候选打分逐字节不变（benchmark/reproduce 16/16），policy snapshot 仅启用时写 `hybrid_backend`/`hybrid_weight`。**剩余（→ §4 多路融合 / 多存储一致性项）**：把 BM25 纳入 RRF 三路融合、写路径把 memory 索引进 ES（`index_status`/`last_indexed_at` 与后台 reindex）、valid_time/branch_status filter 下推到 ES query。
...
- [x] **Query Planner**（query rewrite + entity/keyword hints）+ Need-Retrieval Decision（简单任务跳过检索）。**deterministic、无外部依赖、无网络的 opt-in slice 已完成**：`app/retrieval/query_planner.py` 提供 `plan_query`/`hint_boost`（entity-hint 加成）、`decide_need_retrieval`（无信号的琐碎 query 跳过检索）、`rewrite_query`（把结构化实体 `project.runtime` 展开成 component words `project runtime`，使散文记忆也能匹配——因为 lexical tokenizer 把点分 key 当单 token）。`MEMTRACE_RETRIEVAL_QUERY_PLANNER` 现支持 `off`（默认）/`hints`（仅实体加成，2026-06-30 Step 2）/`full`（hints + query rewrite + need-retrieval decision）。默认 `off` → 候选打分逐字节不变（benchmark/replay 16/16 不变）；`full` 的 rewrite 只作为 lexical/vector 评分输入、保留原始 query，need-retrieval 跳过对 long_context 不生效；policy snapshot 仅在启用时写 `query_planner`/`query_planner_weight`（默认 hash 稳定）。`task_intent` 被检索消费。**仍后置**：BM25/graph 路与 task_intent ranking_profiles（依赖 ES/Neo4j 触发条件，见下方多路融合项）。
- [x] **多跳迭代检索 (Iterative Reconstruction)**：cue→tag/entity→content/evidence，每跳受 token budget 限制。出处：draft §5（MRAgent/AdaMEM 方向）。**deterministic、无外部依赖、默认关闭的 slice 已完成**：`query_planner.derive_hop_cues(...)` 从首轮候选内容抽取实体型 cue（点分 key/路径/标识符），`RetrievalController._select_candidates_multi_hop(...)` 在 `MEMTRACE_RETRIEVAL_MULTI_HOP_HOPS>0`（默认 `0`=单轮）时按 cue 串成 hop query 跑额外检索轮，只追加未见过且 token 预算内的候选（用 packer `estimate_tokens` 累计、超 `token_budget` 即停），把「只靠共享实体（如 `service.gateway`）链接、query 本身不提及」的互补记忆召回；候选带 `hop` provenance（0=直检，>0=多跳）。每跳 cue 数受 `retrieval_multi_hop_max_cues`（默认 4）限制。默认 `hops=0` → 候选与单轮逐字节相同（benchmark/reproduce 16/16 不变）；policy snapshot 仅启用时写 `multi_hop_hops`（默认 hash 稳定），retrieval profile 仅启用时加 `multi_hop_*` metadata。`long_context` 不做多跳。
...
- [x] **小规模 LoCoMo / MemoryArena**（§15 标注「可做小规模，不优先刷榜」）。**已完成首批（2026-06-30，确定性、无网络、不引入外部数据集文件）**：新增 3 个 LoCoMo/MemoryArena 方法学启发的确定性 recall case —— `case_14_long_horizon_recall`（单跳长程召回：早期事实经多步与干扰记忆后仍被召回，no-memory baseline 无法）、`case_15_temporal_knowledge_update`（时序知识更新：仅召回当前值，superseded 历史不泄漏）、`case_16_multi_hop_recall`（多跳：两条互补事实都被检索）。新增通用 `target_recall_hit` / `recall_distractor_leakage` 评测维度与 3 条 acceptance；benchmark 由 13→**16 cases × 6 strategies = 96 rows**、acceptance **16/16**，强制确定性 provider registry 保持可复现。另新增 opt-in 真实大模型问答 bench `app/benchmark/qa_bench.py`（`MemoryRuntime` 真实热路径 seed → `baseline_0` 无记忆 vs `variant_2` 门控 → 同一问题分别问真实 LLM → 评分正确性与记忆增益；env-gated，无 endpoint 时干净跳过），经本地代理 `:4141`（`gpt-5-mini`）实测：门控上下文「This project uses Bun.」→「bun test」，无记忆→「I do not have that information.」。**数据集摄入 harness 已完成（2026-06-30 loop Step 3）**：新增 `app/benchmark/dataset_bench.py` —— 把 LoCoMo/MemoryArena 风格的 JSONL（每行一条 `{id, facts[], probes[]}`：`facts` 可带 `status=superseded`（时序更新）/`branch_status=failed`（死分支干扰），`probes` 带 `recall_markers`/`distractor_markers`）摄入并在**相同 seed** 上对比 plain-vector（`baseline_1` 无门控）vs MemTrace（`variant_2` 门控）的 recall 命中率与 distractor 泄漏率，确定性评分（无 LLM/无网络），输出 `delta.recall_rate_gain` / `delta.distractor_leakage_reduction`。内置 3 条样本 `app/benchmark/data/sample_dataset.jsonl`（可无外部数据运行/测试），更大真实数据集经 `--dataset` / `MEMTRACE_DATASET_PATH` 注入；样本上 plain-vector 泄漏 failed-branch `npm test`（leakage=0.5）而门控隔离（0.0，leakage_reduction=0.5），superseded 端点对两者皆被生命周期过滤。schema 见 `docs/benchmark.md`。standalone，不影响确定性 benchmark 的 16/16。**降级保留**：核心贡献是 Agent Runtime 级记忆治理，全量 LoCoMo/MemoryArena 刷榜仍不做。
- [x] **集成/回归测试补全**：unit（gate/packer/state transition/extractor parser）、integration（API + postgres + migration + retrieval）、e2e（demo agent + benchmark）、regression（failed_branch / workspace_isolation / stale / tool_safety）。**已完成（2026-06-30）**：现有 800+ pytest 已覆盖 unit（`tests/retrieval/test_gate.py` / `test_packer*` / `tests/runtime/test_state_tree.py` / `tests/memory/test_llm_extractor*`）、integration（`tests/api/*` / `tests/storage/test_migrations.py` / retrieval flow）、e2e（demo + `tests/benchmark` 16/16 acceptance）。新增 facade 级回归 `tests/runtime/test_safety_axes_e2e.py`（5 例）直接驱动公开 `MemoryRuntime.retrieve_context`，对四条安全轴（failed-branch 隔离 / workspace 隔离 / stale 排除 / tool-sensitive 拦截）断言「坏记忆不进正向上下文、好记忆进上下文」，与 gate 单测 + benchmark 形成三层防护。
- [x] **Docker Compose 分层（避免开发环境过重）**：`compose.core`（api + postgres）/ `compose.dev`（+ redis + worker）/ `compose.full`（+ es + neo4j + frontend）。**core/dev 已完成**：`docker-compose.yml`（PostgreSQL/pgvector，core）+ `docker-compose.yml -f docker-compose.dev.yml`（+Redis+worker，dev），分层与命令已在 `docs/deployment.md` §Compose layering 明确记录。**full（ES/Neo4j/前端容器）后置至 Phase 5**：runtime 尚未使用 ES/Neo4j（§4 触发门控），不预先 ship 应用无法使用的后端服务（避免 invented infra）；`apps/web` 以独立 Bun dev server 运行而非 compose 服务。出处：draft §12.4 / §11。
...
- [x] **后续候选（本节衍生）**：stale → 「过时警告」降级。**已完成（2026-06-30，默认关闭）。** 新增 `GateConfig.enable_stale_warning`（默认 `False`）+ config `stale_warning_enabled`（默认 `False`，仅 failure-learning 策略 variant_2/variant_3 生效，经 `RetrievalController._gate_config` 注入）。启用后，risk-policy 中**安全的** stale 记忆（secret/destructive/tool_sensitive 已被上游安全底线 hard reject）从 silent `reject "stale"` 改为 `degrade "stale_outdated"`，经共享 `negative_evidence` builder 渲染为 `outdated_warning` 模式（`OUTDATED — ... verify before relying on it: <redacted>`，secret 内容降级为通用模板），作为 warning-only `avoided_attempts` 块注入（ordinary、可被预算丢弃），**绝不进入正向上下文**，故 `case_9 variant_2_excludes_stale_memory` 与默认 benchmark/reproduce 13/13 不变。`NegativeEvidence.mode` / `RetainedNegativeEvidence.mode` 新增 `outdated_warning`。replay/inspect 经同一 builder 一致重建。
...
- [x] **其余 Low 项**：见 §1.1 末条。**2026-06-30 closeout 已复核**：`tool_sensitive_present` 误判、`last_accessed_at` 死字段、报告路径 TOCTOU、naive datetime 健壮性、episodic risk 屏蔽均已修复/确认修复。**2026-06-30 loop Step 1 闭环**：`access_count` read-modify-write 竞态已修（`update_memory` 默认 `preserve_access_bookkeeping=True`，访问簿记唯一权威是 `bump_memory_access`，benchmark restore 显式 opt-out；in-memory/SQL 对称）；CLI/evaluator 判定重复已消除（下沉到 `app/runtime/context_actions.py`，CLI 经 `memtrace_sdk.context_actions` 复用，不触碰 `app.benchmark` 边界）。仅 `raw_event_ids` 反规范化缓存按设计保留（真相源是 `agent_events` join）。
