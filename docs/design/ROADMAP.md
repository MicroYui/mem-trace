...
- [ ] **Query Planner**（query rewrite + entity/keyword hints）+ Need-Retrieval Decision（简单任务跳过检索）。**opt-in micro-slice 部分完成（2026-06-30 loop Step 2）**：新增 deterministic、无外部依赖、无网络的 `app/retrieval/query_planner.py`（`plan_query` / `hint_boost`），从 query + task_intent 抽取**实体型 token**（点分 key `project.runtime`、路径 `src/app.py`、含分隔符/数字的标识符 `api_key`/`v2.0`），对**提及这些实体的候选**施加有界 lexical 加成（`weight * matched/total`，默认 `0.1`、封顶 1.0），使结构化名词排在「仅泛词重叠」候选之前。通过 `MEMTRACE_RETRIEVAL_QUERY_PLANNER=hints` 切换，默认 `off` → 候选打分逐字节不变（benchmark/replay 16/16 不变）；policy snapshot 仅在启用时写 `query_planner`/`query_planner_weight`（与 RRF 同样保持默认 hash 稳定）。`task_intent` 现已被检索消费（此前仅落 access log）。**仍后置**：query rewrite、Need-Retrieval Decision（跳过检索）、BM25/graph 路与 task_intent ranking_profiles（依赖 ES/Neo4j 触发条件）。
